...
def tsv2dict():
    ...
    for line in reader:
        ...
        line["summary"], line["description"], combined_corpus = get_combined_corpus(line)
...
def get_top_k_wrong_files(right_files, br_corpus, java_files):
    ...
    topfifty = sorted(all_files, key=lambda x: x[1], reverse=True)[:len(right_files)]
    return topfifty
...
def calculate_scores(id, buggy_src_file, java_files, raw_corpus, date, bug_report, bug_reports, report_time, match):
    ...
    prev_reports = get_previous_report_by_filename(buggy_src_file, date, bug_reports)
    related_corpus = []
    ...
    mrReport = get_most_recent_report(buggy_src_file, convert_to_datetime(report_time), bug_reports)
    ...
    with open(our_features_path, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([id, buggy_src_file, rVSM_text_similarity, collaborative_filter_score, classname_similarity, bug_fixing_recency_, bug_fixing_frequency_, match])
...
def get_most_recent_report(filename, current_date, dictionary):
    matching_reports = get_previous_report_by_filename(filename, current_date, dictionary)
...
def collaborative_filtering_score(report, filename, dictionary):
    matching_reports = get_previous_report_by_filename(filename, convert_to_datetime(report.get("report_time")), dictionary)